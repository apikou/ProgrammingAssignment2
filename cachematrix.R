##  Computing a matrix inverse can be time consuming especially for big data .The idea here is: if the
##  matrix does not change ( and supposed to be inversible) , to write functions that can  store 
##  (cache)   the first computation of the matrix inverse , to be called later.This will be done by two 
##   functions :
##   makeCacheMatrix() and cacheSolve().
##   makeCacheMatrix() creates a new object MakecacheMatrix that has 2 pieces of imternal data (matrix x 
